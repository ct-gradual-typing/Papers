...
\begin{abstract} 
  ...
  static and dynamic typing called Core Grady that corresponds to our
  model.  Then we develop a gradually typed surface language for Core
  Grady, and show that it can be translated into Core Grady such that
  the gradual guarantee holds. In addition, we show that Siek and
  Taha's gradual simply typed $\lambda$-calculus can be modeled by the
  proposed semantics.  Finally, while a gradual type system allows for
  type casts to be left implicit we show that explicit casts can be
  derived, and using the explicit casts we show that more programs can
  be typed.  For example, we define a typed fixpoint operator for the
  natural numbers that can only be defined due to the explicit casts
  in the gradual type system.
\end{abstract}
