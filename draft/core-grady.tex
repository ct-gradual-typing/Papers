\renewcommand{\CGradydruleSXXArrowKName}{\to}
\renewcommand{\CGradydruleSXXProdKName}{\times}
\renewcommand{\CGradydruleSXXListKName}{\mathsf{List}}
\begin{figure}
  \begin{mdframed}
    \begin{mathpar}
      \cdots \and
      \CGradydruleSXXArrowK{} \and
      \CGradydruleSXXProdK{} \and
      \CGradydruleSXXListK{}
    \end{mathpar}
  \end{mdframed}
  \caption{Subtyping for Core Grady}
  \label{fig:subtyping-core-grady}
\end{figure}

\renewcommand{\CGradydruleTXXvarPName}{\text{var}}
\renewcommand{\CGradydruleTXXBoxName}{[[box]]}
\renewcommand{\CGradydruleTXXUnboxName}{[[unbox]]}
\renewcommand{\CGradydruleTXXsquashName}{[[squash]]}
\renewcommand{\CGradydruleTXXsplitName}{[[split]]}
\renewcommand{\CGradydruleTXXunitPName}{[[Unit]]}
\renewcommand{\CGradydruleTXXzeroPName}{\text{zero}}
\renewcommand{\CGradydruleTXXsuccName}{[[succ]]}
\renewcommand{\CGradydruleTXXncaseName}{[[Nat]]_e}
\renewcommand{\CGradydruleTXXemptyName}{\text{empty}}
\renewcommand{\CGradydruleTXXconsName}{\mathsf{List}_i}
\renewcommand{\CGradydruleTXXlcaseName}{\mathsf{List}_e}
\renewcommand{\CGradydruleTXXpairName}{\times_i}
\renewcommand{\CGradydruleTXXfstName}{\times_{e_1}}
\renewcommand{\CGradydruleTXXsndName}{\times_{e_2}}
\renewcommand{\CGradydruleTXXlamName}{\to_i}
\renewcommand{\CGradydruleTXXappName}{\to_e}
\renewcommand{\CGradydruleTXXLamName}{\forall_i}
\renewcommand{\CGradydruleTXXtypeAppName}{\forall_e}
\renewcommand{\CGradydruleTXXSubName}{\text{sub}}
\renewcommand{\CGradydruleTXXerrorName}{\text{error}}
\begin{figure}
  \begin{mdframed}
    \begin{mathpar}
      \CGradydruleTXXvarP{} \and
      \CGradydruleTXXBox{} \and
      \CGradydruleTXXUnbox{} \and
      \CGradydruleTXXsquash{} \and
      \CGradydruleTXXsplit{} \and
      \CGradydruleTXXunitP{} \and
      \CGradydruleTXXzeroP{} \and
      \CGradydruleTXXsucc{} \and
      \CGradydruleTXXncase{} \and
      \CGradydruleTXXempty{} \and
      \CGradydruleTXXcons{} \and
      \CGradydruleTXXlcase{} \and
      \CGradydruleTXXpair{} \and
      \CGradydruleTXXfst{} \and
      \CGradydruleTXXsnd{} \and
      \CGradydruleTXXlam{} \and
      \CGradydruleTXXapp{} \and
      \CGradydruleTXXLam{} \and
      \CGradydruleTXXtypeApp{} \and
      \CGradydruleTXXSub{} \and
      \CGradydruleTXXerror{} 
    \end{mathpar}
  \end{mdframed}
  \caption{Typing rules for Core Grady}
  \label{fig:typing-core-grady}
\end{figure}

\renewcommand{\CGradydrulerdXXretracTName}{}
\renewcommand{\CGradydrulerdXXretracTEName}{}
\renewcommand{\CGradydrulerdXXretractUName}{}
\renewcommand{\CGradydrulerdXXretractUEName}{}
\renewcommand{\CGradydrulerdXXsuccName}{}
\renewcommand{\CGradydrulerdXXncaseZeroName}{}
\renewcommand{\CGradydrulerdXXncaseSuccName}{}
\renewcommand{\CGradydrulerdXXncaseOneName}{}
\renewcommand{\CGradydrulerdXXncaseTwoName}{}
\renewcommand{\CGradydrulerdXXncaseThreeName}{}
\renewcommand{\CGradydrulerdXXlcaseEmptyName}{}
\renewcommand{\CGradydrulerdXXlcaseConsName}{}
\renewcommand{\CGradydrulerdXXheadName}{}
\renewcommand{\CGradydrulerdXXtailName}{}
\renewcommand{\CGradydrulerdXXlcaseOneName}{}
\renewcommand{\CGradydrulerdXXlcaseTwoName}{}
\renewcommand{\CGradydrulerdXXlcaseThreeName}{}
\renewcommand{\CGradydrulerdXXbetaName}{}
\renewcommand{\CGradydrulerdXXetaName}{}
\renewcommand{\CGradydrulerdXXprojOneName}{}
\renewcommand{\CGradydrulerdXXprojTwoName}{}
\renewcommand{\CGradydrulerdXXetaPName}{}
\renewcommand{\CGradydrulerdXXappOneName}{}
\renewcommand{\CGradydrulerdXXappTwoName}{}
\renewcommand{\CGradydrulerdXXfstName}{}
\renewcommand{\CGradydrulerdXXsndName}{}
\renewcommand{\CGradydrulerdXXpairOneName}{}
\renewcommand{\CGradydrulerdXXpairTwoName}{}
\renewcommand{\CGradydrulerdXXtypeBetaName}{}
\renewcommand{\CGradydrulerdXXtypeAppName}{}
\begin{figure}
  \begin{mdframed}
    \begin{mathpar}
      \CGradydrulerdXXretracT{} \and
      \CGradydrulerdXXretracTE{} \and
      \CGradydrulerdXXretractU{} \and
      \CGradydrulerdXXretractUE{} \and
      \CGradydrulerdXXncaseZero{} \and
      \CGradydrulerdXXncaseSucc{} \and
      \CGradydrulerdXXlcaseEmpty{} \and
      \CGradydrulerdXXlcaseCons{} \and
      \CGradydrulerdXXbeta{} \and
      \CGradydrulerdXXeta{} \and
      \CGradydrulerdXXprojOne{} \and
      \CGradydrulerdXXprojTwo{} \and
      \CGradydrulerdXXetaP{} \and
      \CGradydrulerdXXtypeBeta{}
    \end{mathpar}
  \end{mdframed}
  \caption{Reduction rules for Core Grady}
  \label{fig:reduction-core-grady}
\end{figure}


