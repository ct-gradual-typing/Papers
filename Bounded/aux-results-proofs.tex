...
%% \end{lemma}
%% \begin{proof}
%%   This proof holds by induction on $[[G |- A <~ B]]$ making use of
%%   inversion for type prevision
%%   (Lemma~\ref{lemma:inversion_of_type_precision}).
%% \end{proof}
...
%%   we know that $[[G |- C ~ B]]$ by Lemma~\ref{lemma:type_precision_triangle_consistenty}.  If the latter, then
%%   we obtain $[[B <= C]]$ by transitivity, and $[[G |- B ~ C]]$ by Lemma~\ref{lemma:type_precision_and_consistency}
...
\begin{restatable}[Congruence of Type Consistency Along Type Precision]{lemma}{conTypeConTypePre}
...
\begin{restatable}[Congruence of Subtyping Along Type Precision]{lemma}{conSubTypePre}
