...
One feature Surface Grady has that cannot be found in previous systems
...
definition of \ginline{omega} we are applying \ginline{x} to
itself without an explicit cast.  However, there is one explicit cast
in the definition of \ginline{fix} which applies
\ginline{unbox} to the result of the Y combinator.  If we did not
have this explicit cast, then \ginline{fix} would not type check.
