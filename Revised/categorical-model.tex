...
At this point we would like to introduce error-handling in a familiar
way by means of a monad. To do this, we alter our definition of an
untyped $\lambda$ model to require $\cat{C}$ have all binary (thus all
finite) coproducts, as first proposed by XXX. The maybe monad
$(T, \eta, \mu)$ is then defined as follows:
\begin{center}
  \begin{math}
    \begin{array}{ll}
      T & = - + 1 \\
  ...
  \end{math}
\end{center}
$\split$ now becomes a morphism from $?$ to $S + 1$, and we require
...
\begin{definition}
  ...
  An \textbf{untyped $\lambda$-model with error handling}, $(\cat{C}, ?, \split, \squash)$, is a cartesian closed category with coproducts $\cat{C}$, a distinguished object $?$ and morphisms $\squash : S \mto ?$ and $\split : [[?]] \mto S$ for S equal to $[[? x ?]]$, $[[? --> ?]]$ or $? + ?$, related by the following equations:
  \begin{center}
    \begin{math}
      \begin{array}{rll}
        \squash_S ; \split_S & = \eta_S & \\
        \squash_{S_1} ; \split_{S_2} & = \error_{S_1, S_2} & \textrm{where } S_1 \neq S_2. \\
      \end{array}
    \end{math}
  \end{center}
\end{definition}
